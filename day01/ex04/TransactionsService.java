...
public class TransactionsService {
    ...
    public UUID transact(int id1, int id2, double amount, TransferCategory transfer_type) {
        User sender = list.getUser(id1);
        User recipient = list.getUser(id2);
        transaction1 = new Transaction(sender, recipient, transfer_type, amount);
        list.getUserByUser(sender).transactions.transactionAdd(transaction1);
        list.getUserByUser(recipient).transactions.transactionAdd(transaction1);
